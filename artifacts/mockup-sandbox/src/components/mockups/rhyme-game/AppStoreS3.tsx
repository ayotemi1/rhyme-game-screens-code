...
function StatsScreen() {
  return (
    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(145deg, #7230e8, #6A25D9, #5418bf)", padding: "38px 10px 10px" }}>
      {/* Header */}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 7 }}>
        {[
          { icon: "🎤", val: "889", label: "TOTAL BARS", highlight: false },
          { icon: "🔥", val: "23", label: "DAY STREAK", highlight: true },
          { icon: "⚡", val: "47", label: "TODAY", highlight: false },
          { icon: "🏆", val: "#12", label: "RANK", highlight: false },
        ].map(({ icon, val, label, highlight }) => (
          <div key={label} style={{ background: highlight ? "rgba(251,191,36,0.18)" : "rgba(255,255,255,0.1)", border: `1px solid ${highlight ? "rgba(251,191,36,0.4)" : "rgba(255,255,255,0.15)"}`, borderRadius: 11, padding: "8px 8px 7px" }}>
            <div style={{ fontSize: 13 }}>{icon}</div>
            <div style={{ fontFamily: "sans-serif", fontWeight: 900, fontSize: 18, color: highlight ? "#FBBF24" : "#fff", lineHeight: 1.1 }}>{val}</div>
            <div style={{ fontFamily: "sans-serif", fontSize: 7, color: highlight ? "#FBBF24" : "rgba(255,255,255,0.45)", fontWeight: 700, letterSpacing: 0.5, marginTop: 2 }}>{label}</div>
          </div>
        ))}
      </div>
      {/* Top beats */}
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 52, background: "rgba(20,8,60,0.9)", borderTop: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", gap: 8, padding: "0 10px 6px" }}>
        ...
        <div style={{ width: 22, height: 22, borderRadius: "50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ width: 8, height: 8, borderRadius: 1, background: "#2D1B6E", display: "flex", gap: 2 }}></div>
        </div>
      </div>
    </div>
  );
...
export function AppStoreS3() {
  return (
    <div style={{ width: 390, height: 844, position: "relative", overflow: "hidden", background: "linear-gradient(145deg, #7230e8 0%, #6A25D9 25%, #5418bf 55%, #4313a8 85%, #360ea0 100%)", fontFamily: "'Nunito', sans-serif" }}>
      ...
      {/* Big Mic — peeking from bottom-left */}
      <img
        src={bigmicImg}
        alt="Big Mic mascot"
        style={{ position: "absolute", bottom: 96, left: -48, width: 200, height: "auto", zIndex: 15, filter: "drop-shadow(0 20px 40px rgba(0,0,0,0.5))" }}
      />
